              ...
              <div className="space-y-4">
                {activeTab === 'overview' && stats && (
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div>
                      <h4 className="font-semibold text-base sm:text-lg mb-3 text-gray-800 dark:text-gray-200">Recent Orders</h4>
                      <div className="space-y-2">
                        {stats.recentOrders.slice(0, 5).map((order) => (
                          <div key={order.id} className="bg-gray-50 dark:bg-gray-700 p-2.5 sm:p-3 rounded-lg flex justify-between items-center">
                            <div className="min-w-0 flex-1">
                              <p className="font-medium text-xs sm:text-sm truncate">{order.user.name}</p>
                              <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">{new Date(order.createdAt).toLocaleDateString()}</p>
                            </div>
                            <div className="text-right flex-shrink-0 ml-2">
                              <p className="font-semibold text-xs sm:text-sm text-primary-green">${order.totalAmount}</p>
                              <span className="text-[10px] sm:text-xs text-gray-600 dark:text-gray-400">{order.status}</span>
                            </div>
                          </div>
                        ))}
                      </div>
